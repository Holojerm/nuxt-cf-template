...
/**
 * Share of the canvas the mark covers on the two manifest icons. Lower than
 * the apple-touch coverage on purpose: Android applies a *maskable* icon's own
 * mask shape (circle, squircle, rounded square — the OS chooses), and only the
 * center ~80%-diameter circle is guaranteed to survive every shape. Anything
 * outside that safe zone is a bet on which mask the launcher picked.
 */
const MASKABLE_COVERAGE = 0.8
...
writeFileSync(
  ...
  `${JSON.stringify(
    {
      _comment:
        'Generated by `bun run brand:generate`. Fingerprints the inputs behind the files listed in `generated` below so `bun run brand:check` can fail the build when they drift. Commit it.',
